...
#error "USE_FUNNELLED_COMM not supported"
#endif
	spin=1;
	l1 = num_partitions;
...
void custom_partition_master(int num_partitions, int threads_per_partition, int *affinity, int g_nodal_rank, std::function<void(int)> f)
{
	if(num_partitions>1 || threads_per_partition>1){
		printf("This is CPU Only Hypre. More than one partitions not supported. Set environment variable HYPRE_THREADS=1,1 or enable USE_MPI_EP\n");
		exit(1);
	}
	f(0);
}
